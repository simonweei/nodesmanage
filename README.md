...
3. 在订阅列表创建订阅，从当前全部 VPS 中勾选一个或多个节点。新建 VPS 会自动出现在选择列表中且默认不勾选，删除 VPS 会自动清理其订阅绑定。客户端随订阅自动生成并拥有独立凭据和订阅 Token；创建、编辑、停用或删除会自动发布所有受影响 VPS。面板提供 Sing-box、Mihomo、Shadowsocks（SIP008）和通用 URI 四种订阅格式；Sing-box 二维码使用官方远程配置导入链接，可由 Android 客户端直接扫描。
