...
3. 在订阅列表创建订阅，从当前全部 VPS 中勾选一个或多个节点。新建 VPS 会自动出现在选择列表中且默认不勾选，删除 VPS 会自动清理其订阅绑定。客户端随订阅自动生成并拥有独立凭据和订阅 Token；创建、编辑、停用或删除会自动发布所有受影响 VPS。管理页可直接显示和复制当前订阅地址，只有主动轮换令牌才会使旧地址失效。面板提供 Sing-box、Mihomo、Shadowsocks（SIP008）和通用 URI 四种订阅格式；Sing-box 二维码使用订阅名称生成官方远程配置导入链接，可由 Android 客户端直接扫描。Sing-box 订阅包含 TUN、FakeIP DNS、DNS 劫持、防回环路由、私网直连、国内域名/IP 直连规则和“节点选择”分组，可在官方图形客户端中直接接管系统流量并切换节点；未匹配国内规则的流量默认走所选代理。
